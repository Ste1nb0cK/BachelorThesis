...
H = \omega_{c} a^{\dagger}a + \sum_{k}\omega_{k}b^{\dagger}_{k}b_{k} + \sum_{k}g_{k}\left( a^{\dagger}b_{k} + ab^{\dagger}_{k} \right).
\end{equation}
Where the $a$'s and $b_k$'s satisfy the bosonic commutation relation.
\subsection{Transformation to the Interaction Picture}
The interesting part of the dynamic lies in the last term of \eqref{eq:radiative_hamiltonian} and so we pass to an interaction picture
with the first two terms in the exponential of the transformation:

\begin{equation}\label{eq:H_0_radiative_model}
  H_{0} = \omega_{c} a^{\dagger}a + \sum_{k}\omega_{k}b^{\dagger}_{k}b_{k}
\end{equation}

\begin{align}
  \tilde{\rho}_{tot}(t) = e^{-iH_{0}t} \rho(t)e^{iH_{0}t}
\end{align}
%%% Local Variables:
%%% mode: latex
%%% TeX-master: "../main"
